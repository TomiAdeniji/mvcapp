...
function AddCommentToSocialPostDiscussion(socialPostId) {
	if (busycomment)
		return;
	...
	if (message.val() && !$('#addcomment-error').is(':visible')) {
		...
		busycomment = true;
		$.ajax({
			url: "/QbicleComments/AddCommentToSocialPostDiscussion",
			data: { message: message.val(), socialPostKey: socialPostId },
			type: "POST",
