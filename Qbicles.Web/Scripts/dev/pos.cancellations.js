...
$(function () {
    ...
    $("#order-cancel-cashier,#order-cancel-manager").multiselect({
...
function LoadDataOrderCancellationServerSide() {
    ...
    $("#order-cancel-table").on('processing.dt', function (e, settings, processing) {
        ...
        "pageLength": 10,
        "ajax": {
            "url": '/OrderCancellation/TraderPosOrderCancellations',
            "type": 'POST',
            "data": function (d) {
                return $.extend({}, d, {
        ...
        "columns": [
            ...
            },
            {
                ...
                render: function (value, type, row) {
                    if (row.TotalItems != "0")
                    return "<button class='btn btn-info' onclick=\"ShowOrderCancellationItemDetail('" + row.Key + "')\" ><i class='fa fa-eye'></i> &nbsp; " + row.TotalItems + "</button>";
            ...
            },
            {
                ...
                render: function (value, type, row) {
                    ...
                    if (row.DiscussionKey == "")
                        return "<button class='btn btn-primary' onclick=\"AddCancellationDiscussion('" + row.Key + "')\" ><i class='fa fa-plus'></i>&nbsp; Discuss</button>";
...
function ShowOrderCancellationItemDetail(cancelKey) {
    $cancelKey = cancelKey;

    $("#order-cancel-table").LoadingOverlay("show");
    var ajaxUri = '/OrderCancellation/OrderCancellationItemDetail?cancelKey=' + cancelKey;
    $('#cancel-order-item').empty();
...
function AddCancellationDiscussion(cancelKey) {
    $cancelKey = cancelKey;
    $("#order-cancel-table").LoadingOverlay("show");
    var ajaxUri = '/OrderCancellation/AddOrderCancellationDiscussion?cancelKey=' + cancelKey;
...
function CreateOrderCancellationDiscussion() {
        ...
        if (files && files.length > 0) {
            ...
            UploadMediaS3ClientSide("discussion-image-upload").then(function (mediaS3Object) {
                ...
                if (mediaS3Object.objectKey === "no_image" || mediaS3Object.objectKey === "no-image") {
                    ...
                    SaveOrderCancellationDiscussion(mediaS3Object.objectKey);
                }
            });
        }
        else {
            SaveOrderCancellationDiscussion("");
        }
    }
}


function SaveOrderCancellationDiscussion(objectKey) {
    var discussionModel = {
    ...
    $.ajax({
        type: "post",
        cache: false,
        url: "/OrderCancellation/SaveOrderCancellationDiscussion?cancelKey=" + $cancelKey,
        data: {
