...
function AddPostToPerformanceDiscussion(performanceId) {
    if (busycomment)
        return;
    ...
    if (message.val() && !$('#addcomment-error').is(':visible')) {
        ...
        busycomment = true;
        $.ajax({
            url: "/QbicleComments/AddPostToPerformanceDiscussion",
            data: { message: message.val(), performanceId: performanceId },
            type: "POST",
...
function SetTopic(topicId) {
    setCookie("topic_stream", topicId);
};
