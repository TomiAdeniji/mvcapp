    ...
    public class SQLite
    {
        ...
        public abstract class BaseService<T> where T : class, new()
        {
            ...
            public long Add(T entity)
            {
                ...
                foreach (PropertyInfo i in propertyInfos)
                {
                    ...
                    if (ca != null)
                    {
                        if (!ca.IsIdentity)
                        {
                            ...
                            values.Append($"{(i.GetValue(entity) == null ? "NULL" : $"'{i.GetValue(entity).ToString().Replace("'", "''")}'")},");
                        }
                        else
                        {
                            hasIdentity = true;
                ...
                if (columns.ToString() == string.Empty) return identity;
                columns.Remove(columns.Length - 1, 1); // Remove additional comma(',')
                values.Remove(values.Length - 1, 1); // Remove additional comma(',')

                var qry = new StringBuilder();
                qry.Append(
                    $"INSERT INTO [{entity.GetType().Name}] ( {columns} ) VALUES ( {values} ); SELECT last_insert_rowid();");
            ...
            public void AddRange(IList<T> entities)
            {
                ...
                foreach (T entity in entities)
                {
                    ...
                    if (columns.ToString() == string.Empty) continue;
                    columns.Remove(columns.Length - 1, 1); // Remove additional comma(',')
                    values.Remove(values.Length - 1, 1); // Remove additional comma(',')


                    qry.AppendLine($"INSERT INTO [{entity.GetType().Name}] ( {columns} ) VALUES ( {values} );");
                }
            ...
            public void Update(T entity)
            {
                ...
                foreach (PropertyInfo i in propertyInfos)
                {
                    ...
                    if (ca != null)
                    {
                        ...
                        {
                            columns.Append(
                                $"[{i.Name}] = {(i.GetValue(entity) == null ? "NULL" : $"'{i.GetValue(entity).ToString().Replace("'", "''")}'")},");
                        }
                        else
                        {
                            clause.Append($"[{i.Name}] = '{i.GetValue(entity)}'");
                        }
                    }
                }

                if (columns.ToString() == string.Empty) return;
                columns.Remove(columns.Length - 1, 1);
                var qry = new StringBuilder();
                qry.Append($"UPDATE [{entity.GetType().Name}] SET {columns} WHERE {clause};");
            ...
            public void UpdateRange(IList<T> entities)
            {
                ...
                foreach (T entity in entities)
                {
                    ...
                    if (columns.ToString() != string.Empty)
                    {
                        ...
                        qry.AppendLine($"UPDATE [{entity.GetType().Name}] SET {columns} WHERE {clause};");
                    }
                    #endregion
                }
            ...
            public T GetById(object id)
            {
                ...
                foreach (var pi in pInfos)
                {
                    ...
                    if (pk == null || !pk.IsPrimary) continue;
                    clause.Append($"[{pi.Name}]='{id}'");
                    break;
                }
                ...
                {
                    StringBuilder qry = new StringBuilder();
                    qry.Append($"SELECT * FROM [{entity.GetType().Name}] WHERE {clause}");
                    var entities = ExecuteGet(qry.ToString());
            ...
            {
                T entity = new T();
                return ExecuteGet($"SELECT * FROM [{entity.GetType().Name}]");
            }
            /// <summary>
                ...
                while (reader.Read())
                {
                        ...
                        .Where(p => p.CustomAttributes.FirstOrDefault(x => x.AttributeType == typeof(DbColumnAttribute)) != null).ToList())
                    {
                        ...
                        try
                        {
                            ...
                            if (ca != null)
                            {
                                if (((DbColumnAttribute)ca).Convert)
                                {
        ...
        public class Filter<T> : IFilter<T> where T : class, new()
        {
            ...
            public void Add(Expression<Func<T, object>> memberExpression, object memberValue)
            {
                ...
                if (queryStringBuilder.ToString() != string.Empty)
                    queryStringBuilder.Append(" AND ");

                queryStringBuilder.Append(string.Format(" [{0}] = {1}", NameOf(memberExpression), memberValue == null ? "NULL" : string.Format("'{0}'", memberValue)));
            }

            public string EntityName { get; private set; }

            private readonly StringBuilder queryStringBuilder;
            ...
            public string Query
            {
                ...
                {
                    return
                        $"SELECT * FROM [{EntityName}] {(queryStringBuilder.ToString() == string.Empty ? string.Empty : "WHERE")} {queryStringBuilder};";
                }
            }
