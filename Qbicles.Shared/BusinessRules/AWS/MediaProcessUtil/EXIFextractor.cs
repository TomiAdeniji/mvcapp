...
namespace Qbicles.BusinessRules.AWS.EXIF
{
    public static class RotatedImage
    {

        public static byte[] OrientExifRotatedImage(S3ObjectModel s3Object, string filePath)
        {

            byte[] rawWebP = File.ReadAllBytes(filePath);
            //var stream = new MemoryStream(rawWebP);
            var image = Bitmap.FromStream(new MemoryStream(rawWebP));

            //var image = Bitmap.FromStream(s3Object.ObjectStream);

            var bitmap = new Bitmap(image);
            var stream = new MemoryStream();
            foreach (var i in image.PropertyItems)
            {
            ...
            switch (s3Object.Extension?.ToLower() ?? "png")
            {
                case "bmp":
            ...
            stream.Position = 0;
            byte[] imageByte = new byte[stream.Length + 1];
        ...
        {
            int rotate = 1;
            int.TryParse(orientation, out rotate);
            switch (rotate)
            {
                case 1:
    ...
    public class EXIFextractor : IEnumerable
    {
        ...
        private static PropertyItem CreatePropertyItem(short type, int tag, int len, byte[] value)
        {
            ...
            var bitmap = resources.FirstOrDefault(e => e.EndsWith("decoy.jpg"));
            // Loads a PropertyItem from a Jpeg image stored in the assembly as a resource.
            Assembly assembly = Assembly.GetExecutingAssembly();
            Stream emptyBitmapStream = assembly.GetManifestResourceStream(bitmap);
